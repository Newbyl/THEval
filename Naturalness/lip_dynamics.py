...
def evaluate_videos(video_paths: List[str], output_path: str):
    ...
    with open(output_path, 'w') as outfile:
        outfile.write("Video Path,Std_Over_Time\n")
        ...
        for video_path in tqdm(video_paths, desc="Evaluating Videos", unit="video"):
            if not os.path.exists(video_path):
                ...
                outfile.write(f"{video_path},NaN\n")
                continue
            ...
            std_metric = compute_mouth_diversity(video_path, method='std_over_time')
            metrics_std.append(std_metric if std_metric is not None else np.nan)
            outfile.write(f"{video_path},{std_metric if std_metric is not None else 'NaN'}\n")
            outfile.flush()
...
def main():
    ...
    overall_avg_std = evaluate_videos(video_paths=video_paths, output_path=args.output_txt)
