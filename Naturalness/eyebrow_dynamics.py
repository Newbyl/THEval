...
def compute_micro_expression_intensity(video_path):
...
def evaluate_videos(video_paths, output_path):
    ...
    with open(output_path, 'w') as outfile:
        outfile.write("Video Path,Micro-Expression Intensity\n")
        outfile.flush()
        ...
        with tqdm(total=len(video_paths), desc="Evaluating Videos") as pbar_videos:
            for video_path in video_paths:
                ...
                mei = compute_micro_expression_intensity(video_path)
                if mei is not None:
                    metrics_list.append(mei)
                    outfile.write(f"{video_path},{mei:.6f}\n")
                else:
                    outfile.write(f"{video_path},NaN\n")
                outfile.flush()
...
def main():
    parser = argparse.ArgumentParser(description="Compute eyebrow dynamics for videos.")
    ...
    evaluate_videos(video_paths, output_path=args.output_txt)
