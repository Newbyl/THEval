...
def compute_face_quality(video_path, start_frame, end_frame, metric):
    try:
        ...
        if not frames:
            return None
        # convert full frames to tensor batch (N,3,H,W)
        batch = torch.stack([T.ToTensor()(img) for img in frames]).to(device)
        with torch.no_grad():
            scores = metric(batch)
...
# python global_aesthetic_other.py --video_txt ../input_files/input.txt --output_txt test.txt